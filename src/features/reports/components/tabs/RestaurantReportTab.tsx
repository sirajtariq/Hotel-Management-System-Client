    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-2xs">
          ...
          <div className="text-2xl font-bold text-slate-900 mt-1 font-sans">{formatPKR(data.totalSales ?? data.total_sales ?? 0)}</div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-2xs">
          ...
          <div className="text-2xl font-bold text-amber-900 mt-1 font-sans">{formatPKR(data.totalDiscount ?? data.total_discount ?? 0)}</div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-2xs">
          ...
          <div className="text-2xl font-bold text-emerald-900 mt-1 font-sans">{formatPKR(data.totalTax ?? data.total_tax ?? 0)}</div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-2xs space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(data.orderTypeSplit ?? data.order_type_split ?? []).map((split, idx) => (
            <div key={idx} className="bg-slate-50 border border-slate-200/70 rounded-xl p-4 flex flex-col justify-between">
              <span className="text-xs font-semibold text-slate-500">{split.orderType ?? split.order_type}</span>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {(data.topSellers ?? data.top_sellers ?? []).length === 0 ? (
                <tr>
                ...
                (data.topSellers ?? data.top_sellers ?? []).map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-3 text-slate-600">{item.categoryName ?? item.category_name}</td>
                    <td className="p-3 text-center font-bold text-slate-800">{item.quantitySold ?? item.quantity_sold} Units</td>
                    <td className="p-3 text-right font-mono font-bold text-emerald-700">{formatPKR(item.totalRevenue ?? item.total_revenue ?? 0)}</td>
                  </tr>
                ))
              )}
            </tbody>
