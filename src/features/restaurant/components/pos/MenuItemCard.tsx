...
export function MenuItemCard({ item, onSelect }: MenuItemCardProps) {
  ...
  const priceDisplay = isNaN(price) ? '0' : price.toLocaleString();

  const isAvailable = item.isAvailable ?? item.is_available ?? item.isActive ?? item.is_active ?? true;
  const hasVariations = item.hasVariations ?? item.has_variations ?? false;
  ...
  return (
    <div
      ...
      className={cn(
        ...
        !isAvailable ? 'opacity-60 grayscale cursor-not-allowed border-slate-200' : 'border-slate-200/80 hover:border-indigo-500/50'
      )}
    >
      <div>
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="h-10 w-10 rounded-xl bg-indigo-50 text-indigo-900 flex items-center justify-center font-bold text-sm shrink-0 group-hover:scale-105 transition-transform">
            {imageUrl ? (
              <img src={imageUrl} alt={item.name} className="h-full w-full object-cover rounded-xl" />
            ) : (
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100">
        <div>
          ...
          <span className="text-sm font-extrabold text-slate-900">
            PKR {priceDisplay}
            {hasVariations && <span className="text-xs text-slate-400 font-normal"> +</span>}
          </span>
        </div>
