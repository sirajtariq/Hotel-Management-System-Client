...
export function TenantsPage() {
  ...
  const handleToggleStatus = async (tenant: TenantItem) => {
    try {
      await tenantService.updateTenant(tenant.id, { is_active: !tenant.is_active });
      await fetchData();
    } catch (err) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Hotel Clients</span>
            <div className="h-8 w-8 rounded-lg bg-slate-100 flex items-center justify-center text-slate-700">
              <Building2 className="h-4 w-4" />
            </div>
          </div>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-slate-900">{metrics?.total_tenants ?? tenants.length}</span>
            <span className="text-xs text-slate-500 font-medium">
              ({metrics?.active_tenants ?? tenants.filter((t) => t.is_active).length} active)
            </span>
          </div>
          <p className="text-[11px] text-slate-400 mt-1 font-medium">Platform Onboarded Tenants</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-slate-900">{metrics?.active_tenants ?? 0}</span>
            <span className="text-xs text-emerald-600 font-medium">
              {metrics?.total_tenants ? Math.round((metrics.active_tenants / metrics.total_tenants) * 100) : 100}% Active
            </span>
          </div>
          <p className="text-[11px] text-slate-400 mt-1 font-medium">Operating Hotel Accounts</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs">
          ...
          <p className="text-[11px] text-slate-400 mt-1 font-medium">Recurring Monthly Revenue</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-slate-900">{metrics?.overdue_count ?? 0}</span>
            <span className="text-xs text-amber-600 font-medium">
              ({metrics?.due_soon_count ?? 0} Due Soon)
            </span>
          </div>
          <p className="text-[11px] text-slate-400 mt-1 font-medium">Requires Renewal Attention</p>
        </div>
      </div>
                  ...
                  <tr key={tenant.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="space-y-0.5">
                        {renderQuotaUsage(tenant.current_properties_count, tenant.max_properties, 'Props')}
                        {renderQuotaUsage(tenant.current_rooms_count, tenant.max_rooms, 'Rooms')}
                        {renderQuotaUsage(tenant.current_users_count ?? tenant.users_count, tenant.max_users, 'Users')}
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-4 text-slate-600 font-medium">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>{formatDate(tenant.next_due_date)}</span>
                      </div>
                    </td>
