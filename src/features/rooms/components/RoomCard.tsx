...
export function RoomCard({ room, onStatusChange, onCardClick }: RoomCardProps) {
  ...
  const hkUpper = String(room?.housekeeping_status || '').toUpperCase();
  const isDirtyRoom = statusKey === 'AVAILABLE' && (hkUpper === 'DIRTY' || hkUpper === 'DIRTY_ROOM');

  const borderClass = isDirtyRoom
    ? 'border-l-rose-500 hover:border-rose-500 hover:bg-rose-50/20'
  ...
  return (
    ...
    >
      <div className="space-y-2.5">
        {/* Top Row: Prominent Room Number, Floor Pill & Status Dropdown */}
        <div className="flex items-start justify-between gap-2">
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base font-black text-slate-900 font-mono tracking-tight group-hover:text-indigo-900 transition-colors flex items-center gap-1.5">
                <span className="text-slate-400 font-medium text-xs uppercase tracking-normal font-sans">Room</span>
                <span className="text-indigo-950 font-black text-lg">{room?.roomNumber || 'N/A'}</span>
              </h3>
            ...
            <p className="text-xs font-semibold text-indigo-600/90 mt-0.5 line-clamp-1">
              {roomTypeLabel}
            </p>
          </div>

          <div className="flex items-center gap-1.5 shrink-0" onClick={(e) => e.stopPropagation()}>
            <RoomStatusBadge status={room?.status || 'AVAILABLE'} housekeepingStatus={room?.housekeeping_status} />
